/** @file
  ...
  @authors Jakub Pawlewicz <pan@mimuw.edu.pl>, Marcin Peczarski <marpe@mimuw.edu.pl>, Michał Skwarek
...
/**
 * Zwraca liczbę jednomianów w wielomianie.
 * @param[in] p : wielomian
 * @return liczba jednomianów w wielomianie
 */
static inline size_t PolyLength(const Poly *p) {
    return p->size;
}
...
/**
 * Tworzy jednomian, który jest współczynnikiem (jednomian stały).
 * @param[in] c : wartość współczynnika
 * @return jednomian
 */
static inline Mono MonoFromCoeff(poly_coeff_t c) {
    return (Mono) {.p = PolyFromCoeff(c), .exp = 0};
}
...
/**
 * Składa wielomiany.
 * Podstawia wielomiany z tablicy q pod kolejne zmienne.
 * @param[in] p : wielomian
 * @param[in] k : rozmiar tablicy q
 * @param[in] q : tablica z wielomianami do składania
 * @return wielomian po operacji złożenia
 */
